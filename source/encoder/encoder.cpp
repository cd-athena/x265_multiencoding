...
bool Encoder::compute_weighted_DCT_energy(x265_picture *pic, x265_frame_texture_t *m_texture)
{
    pixel *src = NULL;
    ...
    else if (pic->bitDepth == 8 && X265_DEPTH > 8)
    {
        ...
        primitives.planecopy_cp(yChar, pic->stride[0] / sizeof(*yChar), m_inputPic[0], pic->stride[0] / sizeof(*yChar),
                                pic->width, pic->height, shift);
        src = m_inputPic[0];
    }
    else
    {
        ...
        if (pic->bitDepth > X265_DEPTH)
        {
            primitives.planecopy_sp(yShort, pic->stride[0] / sizeof(*yShort), m_inputPic[0], pic->stride[0] / sizeof(*yShort),
                                    pic->width, pic->height, shift, mask);
        }
        else /* Case for (pic.bitDepth < X265_DEPTH) */
        {
            primitives.planecopy_sp_shl(yShort, pic->stride[0] / sizeof(*yShort), m_inputPic[0], pic->stride[0] / sizeof(*yShort),
                                        pic->width, pic->height, shift, mask);
        }
        src = m_inputPic[0];
    }
    ...
    for (int blockX = 0, ctuIdx = 0; blockX < maxWidth; blockX += maxBlockSize)
    {
        for (int blockY = 0; blockY < maxHeight; blockY += maxBlockSize, ctuIdx++)
        {
            ...
            int32_t *weightedDCTCoeff = X265_MALLOC(int32_t, maxBlockSize * maxBlockSize);
            if (maxBlockSize == 64)
            {
                int32_t ctuEnergy = 0;
    ...
    if ((pic_in && (!m_param->chunkEnd || (m_encodedFrameNum < m_param->chunkEnd))) || (m_param->bEnableFrameDuplication && !pic_in && (read < written)))
    {
        ...
        if (m_dpb->m_freeList.empty())
        {
            ...
            if (inFrame->create(p, inputPic->quantOffsets))
            {
                ...
                {
                    x265_picture *pic = (x265_picture *)pic_in;
                    compute_weighted_DCT_energy(pic, inFrame->m_frame_texture);
                }
            }
            else
            {
